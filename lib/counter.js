 ...
 * @returns {Array}
 */
TimestampedCounter.prototype.getKeys = function(ts) {
  ...
  if (ts) {
    var now = ts.format(momentFormat);
  } else {
...
/**
 * Increments this counter with 1 and for a given datetime
 *
 * For some use cases, it makes sense to pass in an event object to get more
 * precise statistics for a specific event. For example, when counting page
 * views on a page, it makes sense to increment a counter per specific page.
 * For this use case, the eventObj parameter is a good fit.
 *
 * @param {Date} [ts] - moment.js Date of the event time
 * @param {Object|string} [eventObj] - Extra event information used to
 *   determine what counter to increment.
 * @param {function} [callback] - Optional callback.
 * @returns {Promise} A promise that resolves to the results from Redis. Can
 ...
 * @since 0.6.0
 */
TimestampedCounter.prototype.incrWithDate = function(ts, eventObj, callback) {
    return this.incrbyWithDate(1, ts, eventObj, callback);
};
 ...
 * @since 0.6.0
 */
TimestampedCounter.prototype.incrbyWithDate = function(amount, ts, eventObj, callback) {
    ...
    this._incrby(amount, ts, eventObj, cb);
...
TimestampedCounter.prototype.incrby = function(amount, eventObj, callback) {
  ...
  var deferred = Q.defer();
  var cb = utils.createRedisCallback(deferred, callback);
  // -- null is passed to mean now() for timestamp
  this._incrby(amount, null, eventObj, cb);
  return deferred.promise;
};
...
TimestampedCounter.prototype._incrby = function(amount, ts, eventObj, cb) {
  var keys = this.getKeys(ts);
